...
FilePath: \XianYuApis\xianyu_web\goofish_apis.py
'''
import html
import json
import os
...
from xianyu_web.message.types import Price, DeliverySettings
from xianyu_web.utils.goofish_utils import generate_sign, trans_cookies, generate_device_id
...
def _request_optional(action: Callable[[], requests.Response], desc: str) -> Optional[requests.Response]:
    """登录前置请求有时会被网络丢弃，失败时继续走主流程。"""
    try:
        return action()
    except requests.RequestException as exc:
        print(f'[qrcode_login] {desc} 失败，继续尝试登录：{exc}')
        return None
...
def _show_qrcode_in_browser(
        qr_url: str,
        output_path: Optional[Path] = None,
        open_browser: bool = True) -> Optional[Path]:
    output_path = output_path or (_HERE / 'exports' / 'runtime' / 'qrcode_login.html')
    ...
    try:
        ...
        qr = qr_lib.QRCode(border=3, image_factory=SvgPathImage)
        qr.add_data(qr_url)
        qr.make(fit=True)
        image = qr.make_image(attrib={'class': 'qr-code'})
        svg_data = image.to_string()
        svg = svg_data.decode('utf-8') if isinstance(svg_data, bytes) else str(svg_data)
    except Exception as exc:
        print(f'[qrcode_login] 生成 SVG 二维码失败，只写入登录 URL：{exc}')
        svg = ''

    escaped_url = html.escape(qr_url, quote=True)
...
<head>
  ...
  <style>
    ...
    main {{
      ...
      border-radius: 28px;
      background: #fffaf0;
      box-shadow: 0 24px 80px rgba(31, 41, 51, .16);
      text-align: center;
    }}
    .qr-wrap {{
      display: inline-grid;
...
<body>
  <main>
    <h1>用闲鱼 App 扫码登录</h1>
    <p>打开闲鱼 App，进入扫一扫。确认登录后，终端会继续导出聊天记录。</p>
    <div class="qr-wrap">{svg or '<p>二维码生成失败，请点击下方链接。</p>'}</div>
    <p class="fallback">如果扫码失败，打开这个链接：<br><a href="{escaped_url}">{escaped_url}</a></p>
  </main>
</body>
</html>
'''
    output_path.write_text(html_text, encoding='utf-8')
    print(f'[qrcode_login] QR page: {output_path}')
    if open_browser:
        try:
            if sys.platform == 'darwin':
                subprocess.run(
                    ['open', str(output_path.resolve())],
                    check=False,
            ...
            print(f'[qrcode_login] 自动打开二维码页面失败，请手动打开上面的 QR page：{exc}')
    return output_path
...
def build_initial_cookies() -> dict:
    ...
    _request_optional(
        lambda: s.get('https://log.mmstat.com/eg.js', timeout=5),
        '获取 mmstat 初始 cookie',
    )
    ...
    for api in ['mtop.taobao.idlehome.home.webpc.feed',
        ...
        _request_optional(
            lambda api=api: s.post(
                ...
                data='data=%7B%7D', headers=_MTOP_HEADERS, timeout=5,
            ),
            f'预热接口 {api}',
        )
...
def qrcode_login(poll_interval: float = 3.0, timeout: float = 120.0,
                 ...
                 qrcode_output_path: Optional[str] = None,
                 open_qrcode: bool = True) -> 'XianyuApis':
    ...
    if show_qrcode:
        output_path = Path(qrcode_output_path).expanduser() if qrcode_output_path else None
        _show_qrcode_in_browser(qr_url, output_path, open_qrcode)
